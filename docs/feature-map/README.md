...
| Browser panel | Live in-panel browser the agent drives; **Annotate** (desktop) picks page elements, takes a note per element in the panel, and adds them to chat with the agent's `eN` refs (`browser:annotate` IPC, `electron/browser-annotate.js`) | Right panel → **Browser** | `components/WebPreviewPanel.tsx` | `handlers/messaging.py` | `GET,POST /api/browser/view`, `POST /api/browser/command`, `POST /api/browser/command-result` |
...
| Kiro sign-in | Sign in to the Kiro identity agents run as, without a kiro-cli login: Google/GitHub via loopback (local) or device code (remote), Builder ID and company SSO via device code; signed-in summary with sign-out; explicit "sign-in expired" state (no silent fallback to kiro-cli login) | Settings → Overview → **Kiro sign-in** card (Search Everywhere: `overview.kiro-sign-in`); chat error row "not signed in" → **Sign in to Kiro** deep link. The full-screen `KasLoginGate` form is still not mounted at the app root | `pages/settings/KiroSignInCard.tsx`, `components/KasLoginGate.tsx`, `pages/chat/ErrorCard.tsx` | `handlers/kas_login.py`, `auth/service.py` (`status()`), `auth/refresh.py` (`RefreshRejected`) | `GET /api/kas-login`, `POST /api/kas-login/device`, `POST /api/kas-login/loopback`, `POST /api/kas-login/poll`, `POST /api/kas-login/cancel`, `POST /api/kas-login/logout` |
...
| Startup feature video | One short clip introducing a new feature, shown once on the launch after it ships and retired permanently by a `seen` or `dismissed` verdict; yields the whole launch to release notes, an update popup or first-run onboarding, and is skipped in incognito/temporary sessions. A clip is played either from the release folder on disk or streamed from the CDN, and the server is asked whether it is reachable before the dialog opens. A cached clip needs no such check -- the catalog already proved its files are on disk | Appears by itself at startup, with no navigation, on the first launch where nothing else claimed the screen and the catalog still has an entry this install has not used | `components/StartupVideoModal.tsx`, `components/startupVideoGate.ts` (sequencing policy), mounted app-wide in `App.tsx` | `feature_videos.py` | `GET /api/feature-videos/next`, `POST /api/feature-videos/feedback` |
| Feature-video cache readout | How much of the current release's clips is on disk, which one is downloading, and one control to fetch them all now. Read-only apart from that control; the control is absent when policy forbids downloads, and the whole row is absent on a gateway whose `status` predates the cache fields | `/settings?tab=chat`, in the Messages card beside Feature Tips | `pages/settings/ChatPanel.tsx` | `feature_videos.py` | `GET /api/feature-videos/status` |
