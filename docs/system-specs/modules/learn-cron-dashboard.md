...
`config.md`): only a ghost carries `motions` (a built-in per-state animation) and
`sounds` (a synthesized per-state preset), because a picture is static and silent
and a pack ships its own per-state art AND its own per-state audio. A reaction
named on a picture or a pack payload is stripped rather than refused, as is the
retired `expressions` key, so a version-skewed or hand-written record never costs
the crew its face. The
