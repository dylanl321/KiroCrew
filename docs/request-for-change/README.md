...
was added later still and re-verified against `2ff4ce819`; the
...
| [rfc-conductor-work-ledger.md](rfc-conductor-work-ledger.md) | `partial` | Phases 1, 2 and 2.5 are on main; verified at `c02cdd67c`. `src/kiro_crew/work_ledger.py` is the store ([#8855](https://github.com/kirodotdev/KiroCrew/pull/8855)); `src/kiro_crew/mcp_work.py` plus `dashboard/handlers/work_ledger.py` are the four tools and their `/api/work-ledger` routes, and `_MANAGED_MCP_SERVERS` carries the `opt_in` `kirocrew-work` entry ([#9152](https://github.com/kirodotdev/KiroCrew/pull/9152)); `agent.py` installs `kirocrew-worker` and `kirocrew-ledger-conductor`, and `kirocrew-conductor` / `kirocrew-pipeline-conductor` deliberately carry no work-ledger grant ([#9277](https://github.com/kirodotdev/KiroCrew/pull/9277)). Not started: the wake gate (`probes/__init__.py` `build` still maps exactly one kind, `gh-pr`), the sibling digest, worker-to-worker channels, and the Crew page surfaces. `goal-conductor/SKILL.md` and `goal-conductor/scripts/ledger_entry.py` are untouched, so an un-migrated conductor still learns worker state by `session_read_message` and still encodes each item into a `session_ledger` `artifacts` value |
...
| [rfc-session-address-model.md](rfc-session-address-model.md) | `partial` | Phase 0 shipped ([#1366](https://github.com/kirodotdev/KiroCrew/pull/1366) plus four follow-ups): a chat-app conversation opened in a dashboard tab joins that conversation instead of being copied into a second one, the 30-second reconciler is deleted, and `has_dashboard_surface` replaced the name-prefix capability tests with observed state — for one surface and one boolean. Phases 1–4 unstarted, verified at `2ff4ce819`: the identity still encodes origin, at least **7** overlapping classifiers and dozens of conversion sites remain, `session_map.json` has no version envelope, capability is still one boolean that a `dashboard:` prefix short-circuits regardless of which surface the turn arrived from, and an unbound channel tab still starts a second session. Revision 3 adds the surface model (dashboard as permanent host, channels as attachments, per-turn ingress) |
...
that owns the shipped behaviour. One is `superseded`: its shipped feature was
removed, and the row names the spec that records the retirement.
