...
Every claim below was measured at `2ff4ce819`. Citations name symbols rather than line numbers: this document's line citations rotted twice in three weeks (once between `fa6261c4f` and `2ff4ce819`, and once when `session.py` shrank from 6489 lines to under 2700 after the re-audit), and a symbol survives the refactor that moves the line. Paths are relative to `src/kiro_crew/`.
...
| `can_inject_turn` | `subagent.py` · `_notify_orphan` | orphan notice as a turn, or fall back to a DM digest |
