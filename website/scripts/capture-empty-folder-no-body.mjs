...
async function newPage(context, { board = false } = {}) {
...
async function main() {
  ...
  const list = await newPage(listContext)
  ...
  const board = await newPage(boardContext, { board: true })
  ...
  await board.screenshot({
    ...
    clip: { x: bx, y: 96, width: Math.min(1400 - bx, 420), height: 940 },
  })
