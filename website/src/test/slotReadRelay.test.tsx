...
import dashboardReducer, { addSlotOptimistic, removeSlotOptimistic, markSlotRead, markSlotUnread, remoteSlotRead, restoreUnreadSince, MANUAL_UNREAD } from '../store/dashboardSlice'
...
describe('slot_read over the dashboard socket', () => {
  ...
  it('an orphan watermark is pruned at boot, not restored as an invisible shield', () => {
    // This window persisted watermarks for X and Z, then a sibling window
    // cleared X from the shared unread list while this tab was unloaded: the
    // badge is gone, so restoring X's watermark would silently shield X from
    // every future remote clear.
    sessionStorage.setItem('mc-unread-since', JSON.stringify({ 'orphan-X': MANUAL_UNREAD, 'alive-Z': '2026-01-01T00:00:05Z' }))
    localStorage.setItem('mc-unread-slots', JSON.stringify(['alive-Z']))
    const restored = restoreUnreadSince()
    expect(restored).toEqual({ 'alive-Z': '2026-01-01T00:00:05Z' })
    // The prune is written back, so the orphan cannot resurrect on the next boot.
    expect(JSON.parse(sessionStorage.getItem('mc-unread-since') ?? '{}')).toEqual({ 'alive-Z': '2026-01-01T00:00:05Z' })
  })
...
describe('hidden-tab reveal relay (single store end-to-end)', () => {
  ...
  it('a visible-but-unfocused window relays nothing on arrival (occluded window is not reading)', () => {
    ...
    // With focus back, the same arrival relays as before.
    act(() => {
      ws.simulateMessage({ type: 'chat_message', data: { slot: ACTIVE, role: 'assistant', content: 'again', ts: '2026-09-10T00:00:11Z' } })
    })
    expect(sentReadFrames(ws)).toEqual([JSON.stringify({ type: 'slot_read', slot: ACTIVE, read_ts: '2026-09-10T00:00:11Z' })])
  })
