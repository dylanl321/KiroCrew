...
describe.each(['svg', 'lottie', 'sprite'] as const)('sample-%s', (format) => {
  ...
  it('carries a done cue the pack can be heard through', () => {
    const { bundle, manifest } = load(`sample-${format}`)
    const sounds = (manifest as unknown as { sounds?: Record<string, string> }).sounds
    expect(sounds?.done).toBe('done.wav')
    const wav = bundle.files['done.wav'] as string
    // Base64 of a real RIFF/WAVE header, and small enough not to bloat the repo.
