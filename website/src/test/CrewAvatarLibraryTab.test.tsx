...
vi.mock('../components/appearancePacks/PackAvatar', () => ({
  default: ({ id, size }: { id: string; size: number }) => (
    <div data-testid={`pack-avatar-stub-${id}`} data-size={String(size)} />
  ),
}))

/** The cache the tab invalidates. Stubbed whole: the real module reaches the
 *  detail route, which this pane's own fixtures do not serve. */
vi.mock('../lib/appearancePacks/detailCache', () => ({
  invalidatePackDetail: (...args: unknown[]) => mockInvalidate(...args),
  loadPackDetail: () => Promise.resolve({ animations: {}, sounds: {} }),
}))
