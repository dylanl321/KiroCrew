...
interface DashboardState {
   ...
   *  deliberate note to self answers only to this window. The whole map is
   *  persisted per-tab and restored with the badges it protects: a sibling
   *  window's trailing relay is still in flight across THIS window's reload,
   *  and a badge restored without its watermark would accept any relay —
   *  wrongly clearing a message the reader never saw. Watermark lifetime
   *  never exceeds badge lifetime (pruned at boot, deleted on clear), so a
   *  stale watermark cannot outlive the badge it guards. */
...
/** unreadSince survives reload: a badge restored without its watermark would
 *  accept ANY relayed read, and a sibling window's trailing relay is still in
 *  flight across this window's reload — so restoring one without the other
 *  reopens the race the watermark exists to close. The map persists to
 *  per-tab sessionStorage, matching its state scope — each window's Redux
 *  holds only the watermarks minted in that window, so a shared localStorage
 *  key would let window A's write clobber the map window B just persisted.
 *  sessionStorage restores this window's own map and is untouchable by
 *  siblings. */
const persistUnreadSince = (unreadSince: Record<string, string>): void => {
  try { sessionStorage.setItem('mc-unread-since', JSON.stringify(unreadSince)) } catch { /* SecurityError / quota */ }
}
...
/** Boot restore for unreadSince (exported for tests). Reconciles this
 *  window's persisted watermarks against the shared unread list: a watermark
 *  whose badge a sibling window cleared while this tab was unloaded is an
 *  orphan — invisible in the UI, yet it would shield its slot from every
 *  future remote clear. Orphans are dropped and the pruned map is written
 *  back, so watermark lifetime never exceeds badge lifetime. */
export const restoreUnreadSince = (): Record<string, string> => {
  try {
    const since = JSON.parse(sessionStorage.getItem('mc-unread-since') ?? '{}') as Record<string, string>
    let badges: string[]
    try { badges = JSON.parse(localStorage.getItem('mc-unread-slots') ?? '[]') as string[] } catch { badges = [] }
    const live = Object.fromEntries(Object.entries(since).filter(([k]) => badges.includes(k)))
    if (Object.keys(live).length !== Object.keys(since).length) persistUnreadSince(live)
    return live
  } catch { return {} }
}
...
const initialState: DashboardState = {
  ...
  unreadSince: restoreUnreadSince(),
  slotsLoaded: false,
  updateProgress: null,
  ...
  if (drained.length !== unread.length) {
    ...
    if (state.unreadSince) {
      ...
      for (const k of unread) if (!liveKeys.has(k)) {
        if (state.unreadSince[k] !== undefined) dropped = true
        delete state.unreadSince[k]
      }
      if (dropped) persistUnreadSince(state.unreadSince)
    }
  ...
  reducers: {
    ...
    removeSlotOptimistic(state, action: PayloadAction<string>) {
      ...
      if (state.unreadSince?.[action.payload] !== undefined) {
        ...
        persistUnreadSince(state.unreadSince)
      }
