...
>(
  ...
  async (arg, { dispatch, getState, rejectWithValue }) => {
    ...
    // Watermark = the slot's server-minted last_ts when known, else the
    // relay goes out with NO watermark — receivers then keep any badge that
    // recorded a watermark of its own (covering nothing is the conservative
    // default). Client time is never minted here: windows disagreeing about
    // the same message would strand badges against valid relays.
    // Optional-chained like the slotRun guard below: a partial preloaded
    // test state can omit the dashboard slice, and throwing here would
    // abort the switch fetch itself.
    const _slotTs = (getState() as RootState).dashboard?.slots?.find(s => s.key === key)?.last_ts
    emitSlotRead(key, _slotTs)
