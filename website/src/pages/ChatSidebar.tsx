...
function ChatSidebar({
  ...
  const renderColumnFolder = (folder: ChatFolder, columnId: string, colSlotKeys: Set<string>, dragHandleProps?: React.HTMLAttributes<HTMLElement>, forceCollapsed?: boolean): React.ReactNode => {
    ...
    // No body is rendered when the column copy holds nothing (see the note in
    // renderFolderBlock), so emptiness costs one row and needs no collapse state.
        ...
        <div
          ...
          {...(emptyBody
            ...
            : {
              ...
              onKeyDown: (e: React.KeyboardEvent) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleColumnCollapse(columnId, folder) } },
            })}
        >
  ...
  const renderFolderHeader = (folder: ChatFolder, dragHandleProps?: React.HTMLAttributes<HTMLElement>, emptyBody = false) => {
    ...
    // `renderFolderBlock` decides whether a body is rendered; this row's own
    // `count` is the second opinion on the same question. Requiring both means a
    // drift between them (a future hidden-reveal or stale-section filter that
    // renders fewer nodes than the row counts) fails toward a pressable row that
    // shows its count, never toward an inert row claiming to hold something.
    const emptyRow = emptyBody && count === 0
  ...
  const renderFolderBlock = (folder: ChatFolder, depth: number, visited = new Set<string>(), dragHandleProps?: React.HTMLAttributes<HTMLElement>, forceCollapsed = false): React.ReactNode[] => {
    ...
    // A folder with nothing in it renders NO body: not a collapsed one, not an
    // empty one. It used to render a body whose only content was a "New chat in
    // <name>" placeholder, so every empty folder cost a second row, and a tree of
    // area folders spent most of the sidebar's height on rows holding nothing.
    // That placeholder existed because the header's create control was invisible
    // until hover; the header now shows it at rest (see `renderFolderHeader`), so
    // the placeholder was a second spelling of a control the row already has.
    // Dropping the body rather than collapsing it is why there is no per-folder
    // expansion state here: nothing is hidden, so nothing needs re-reaching.
