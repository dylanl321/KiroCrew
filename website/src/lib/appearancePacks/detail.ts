 ...
 *
 * Pure data and pure functions — no React, no fetch. The renderer and the sound
 * path both read from here, so "which slot is this state" cannot drift between
 * the frame that is drawn and the cue that plays with it.
 */
import type { AnimationFormat, SpriteConfig } from './types'

/** One slot's art, as the detail route inlines it: the bytes plus the format
 *  that decides which player draws them. */
export interface PackSlotArt {
  content: string
  format: AnimationFormat
}

/** Which states this pack carries a SOUND for. The detail route reports
 *  presence only (`{done: true}`); the bytes come from the sound route. */
...
export function packDetailFrom(payload: unknown): PackDetail {
  const empty: PackDetail = { animations: {}, sounds: {} }
  ...
  const spriteRaw = p.sprite
  const sprite =
    spriteRaw && typeof spriteRaw === 'object' && !Array.isArray(spriteRaw)
      ? (spriteRaw as SpriteConfig)
      : undefined
  const sounds: PackSounds = {}
  const soundsRaw = p.sounds
  if (soundsRaw && typeof soundsRaw === 'object' && !Array.isArray(soundsRaw)) {
    const s = soundsRaw as Record<string, unknown>
    // Presence only, and TRUTHINESS is not enough: the route reports `true`, and
    // reading anything truthy as "has a sound" would make `{"done": "chime"}`
    // from an older or newer server fetch a route that answers 404.
    for (const state of ['working', 'done', 'error'] as const) {
      if (s[state] === true) sounds[state] = true
    }
  }
  return { animations, sprite, sounds }
}
