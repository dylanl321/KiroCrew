...
def validate(doc: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for i, r in enumerate(records):
        ...
        if r.get("feature") not in FEATURE_TITLES:
...
def cross_slug_duplicates(records: list[Any]) -> list[str]:
    ...
    for r in records:
        ...
        key = (
            str(r.get("start_url")),
            str((r.get("preconditions") or {}).get("seed")),
            str(r["component"]),
        )
...
def render(doc: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "",
        "Areas the readers could not place in the closed registry; each was filed under the closest",
        "existing slug for now. Adding a slug is a one-line change to the `FEATURES` registry in",
        "`scenarios.py` (mirrored by `FEATURE_TITLES` here) plus a row in `docs/build/gui-user-test.md`.",
        "",
        "| Slug | Title | Why |",
        "|---|---|---|",
    ]
    for pf in doc.get("proposed_features") or []:
        lines.append(
