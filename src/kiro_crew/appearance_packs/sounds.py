...
its manifest. That is what makes the tier self-contained: the crew record is
``{"kind": "pack", "id": ...}`` and nothing else, because every per-state answer
lives in the pack.
...
#: bundle importer applies the same cap, so an oversized cue is refused where the
#: user can see it rather than silently dropped later.
...
def read_sound(text: Any) -> tuple[tuple[bytes, str] | None, str]:
    ...
    returns, and the bundle importer refuses exactly what it rejects -- which is
    ...
    An importer refuses at the boundary and has to tell the person who picked the
    file which mistake they made: "too long a sound" and "not a sound at all"
    have different fixes. Returning both from one function is what keeps the
