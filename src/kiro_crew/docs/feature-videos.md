...
For a mirrored or air-gapped install, set `KIROCREW_FEATURE_VIDEOS_MANIFEST_URL` in
the gateway's environment to point at your own copy. It must be `https`, and the
signature is still checked — a mirror can relocate the files, not introduce new
ones. This is an environment variable and not a `config.json` key on purpose: the
