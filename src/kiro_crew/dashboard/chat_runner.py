...
def _tool_meta(event: "LLMEvent") -> dict[str, str] | None:
    ...
    return {
        ...
        # Trusted tool identity from ``_meta.kiro`` (empty when the backend sent
        # none). The dashboard's title derivation reads these for MCP calls so a
        # replayed row does not have to parse ``@server/tool`` out of the title;
        # both are optional there, so an older row without them still renders.
        "tool_name": "",
        "mcp_server": "",
