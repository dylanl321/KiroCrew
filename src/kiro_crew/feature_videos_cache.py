...
def _receipt_path(folder: Path, name: str) -> Path:
    return folder / f"{_RECEIPT_PREFIX}{name}{_RECEIPT_SUFFIX}"


def record_verified(folder: Path, name: str, sha256: str) -> bool:
...
def _remove_release(release: str, path: Path) -> bool:
    ...
    Re-checked at the point of deletion, not only when the list was built:
    *path* must be a real directory (never a link) whose canonical path is
    ``<canonical root>/<release>``. ``rmtree`` is irreversible, so this is the
    one place where "inside the cache" has to be proven immediately before the
    call rather than inferred from an earlier scan.
    """
    try:
        if not stat.S_ISDIR(os.lstat(path).st_mode):
            raise manifest_mod.CacheDirRefused(f"not a real directory: {path}")
        if path.resolve(strict=True) != manifest_mod.checked_cache_root() / release:
            raise manifest_mod.CacheDirRefused(f"resolves outside the cache: {path}")
        shutil.rmtree(path)
...
def resolve_served_path(release: str, name: str) -> "tuple[Path, str] | None":
    ...
    This resolves; it does not open. The route opens the returned path ONCE with
    no-follow semantics and streams from that descriptor (:func:`_open_served`),
    so a file swapped for a link between this check and the open is refused by
    the open itself, and a swap after the open changes nothing the response
    reads. Residual, stated: a local process with the user's own permissions can
    still replace the FILE's bytes in place; the receipt certifies what was
    installed, not what is read back, and the user doc says so.
    """
...
def _open_served(path: Path) -> "tuple[int, int] | None":
    """Open one resolved cache file for streaming: ``(fd, size)``, or None.

    The open IS the final check. ``open_file_no_reparse`` refuses a link at the
    leaf in the same operation that opens it (``O_NOFOLLOW`` on POSIX, the
    reparse-point flag on Windows), and the type and size are read off the
    descriptor with ``fstat``. Nothing after this reopens the path: a file swapped
    for a link between :func:`resolve_served_path` and here is refused here, and
    a swap after here changes nothing the response reads. Non-blocking, so a
    planted FIFO is rejected by ``fstat`` instead of blocking the open.
    """
    try:
...
async def api_feature_video_file(request: web.Request) -> web.StreamResponse:
    ...
    The body is streamed from a descriptor opened once with no-follow semantics
    ...
    opened = await asyncio.to_thread(_open_served, path)
    if opened is None:
        raise web.HTTPNotFound()
    fd, size = opened
    try:
