...
def fence_verdict(
    ...
    interface-table sweep from ``fence_self_addresses``). Unparsed
...
# ID_VALID re-check (notification-reuse race), allow = CONTINUE.
#
# Name contract: the text is import-free (the launcher imports everything
# pre-isolation, module-level — see the #8151 note in the template header),
# so the header must bind: os, sys, _struct, _fence_socket, _fence_fcntl,
# _fence_ip, _fence_threading, _FENCE_PORTS, _FENCE_SELF, _fence_nfd.
